...
def gla_return():
    ...
    results = session.query(gdp_lit_aid.country, gdp_lit_aid.year, gdp_lit_aid.gdp_per_capita, gdp_lit_aid.literacy_rate, gdp_lit_aid.net_aid_received).all()
    list_of_gla = []
    for result in results:
        dictt = {'country': result[0],
        'year': result[1],
...
def health_return():
    ...
    results = session.query(health.year, health.ida_only, health.lower_middle_income, health.low_middle_income, health.health).all()
    json_list = []
    for result in results:
        da_dict = {'year':result[0],
